...
export const vTooltip: Directive<HTMLElement, string> = {
    mounted(target: HTMLElement, binding: DirectiveBinding<string>) {
        ...
        target.addEventListener('mouseenter', (e) => show(text, e as MouseEvent))
        target.addEventListener('mousemove', (e) => updatePosition(e as MouseEvent))
        target.addEventListener('mouseleave', hide)
        target.addEventListener('focus', (e) => show(text, e as unknown as MouseEvent))
        target.addEventListener('blur', hide)
    },

    updated(target: HTMLElement, binding: DirectiveBinding<string>) {
        // Met à jour le texte si la valeur de la directive change
        if (binding.value !== binding.oldValue && el && el.style.display !== 'none') {
