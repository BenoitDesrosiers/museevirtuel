...
const estTemoin = computed(() => props.classe.temoin?.id === userId.value);

function estMessageTemoin(echange: Echange): boolean {
    return echange.auteur_id === props.classe.temoin?.id;
}
...
function submit() {
    form.post(`/cours/${props.cours.id}/classes/${props.classe.id}/echanges`, {
        preserveScroll: true,
        onSuccess: () => {
            form.reset();
            scrollToBottom();
...
<template>
    <AppLayout>
        <Head :title="$t('echanges.page_title', { n: classe.numero })" />
        ...
        <div class="flex flex-col gap-6 p-6">
            ...
            <Heading
                :title="$t('echanges.page_title', { n: classe.numero })"
                :description="`${cours.code} · ${cours.nom_cours}`"
            />

            <!-- Témoin info -->
            <Card v-if="classe.temoin">
                <CardContent class="py-3">
                    <div class="flex items-center gap-3">
                        <span class="bg-primary/10 text-primary flex h-8 w-8 items-center justify-center rounded-full text-xs font-medium">
                            {{ classe.temoin.prenom[0] }}{{ classe.temoin.nom[0] }}
                        </span>
                        <div>
                            <p class="text-sm font-medium">{{ classe.temoin.prenom }} {{ classe.temoin.nom }}</p>
